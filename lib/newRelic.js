...
export async function queryAnalytics(userId, videoId = undefined) {
  ...
  const nrqlQuery = `
    ...
    SELECT
      ...
      filter(sum(playtimeSinceLastEvent/60000), WHERE ranking>0) as adFreeStreaming
  ...
  for (const row of results) {
    ...
    chart.uniqueViewers.push(Number(row.uniqueViews))
    ...
    table.push({
      ...
      uniqueViewers: Number(row.uniqueViews),
